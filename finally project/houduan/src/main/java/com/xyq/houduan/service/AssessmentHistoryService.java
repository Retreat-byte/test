...
public class AssessmentHistoryService {
    ...
    public PageResponse<AssessmentHistoryResponse> getAssessmentRecords(
            String username, int page, int size, String startDate, String endDate, AssessmentType type) {
        User user = userRepository.findByPhoneAndDeletedFalse(username)
            .orElseThrow(() -> new RuntimeException("用户不存在"));
    ...
    public AssessmentStatisticsResponse getAssessmentStatistics(String phone, String startDate, String endDate) {
        ...
        // 按类型统计
        long apeskCount = assessments.stream().filter(a -> a.getType() == AssessmentType.APESK_PSTR).count();
        long sasCount = assessments.stream().filter(a -> a.getType() == AssessmentType.SAS).count();
        long sdsCount = assessments.stream().filter(a -> a.getType() == AssessmentType.SDS).count();
        long baiCount = assessments.stream().filter(a -> a.getType() == AssessmentType.BAI).count();
        long psqiCount = assessments.stream().filter(a -> a.getType() == AssessmentType.PSQI).count();
        long dass21Count = assessments.stream().filter(a -> a.getType() == AssessmentType.DASS21).count();
        long scl90Count = assessments.stream().filter(a -> a.getType() == AssessmentType.SCL90).count();
    ...
    private AssessmentHistoryResponse convertToAssessmentHistoryResponse(AssessmentHistory assessmentHistory) {
        ...
        response.setType(assessmentHistory.getType().getCode());
