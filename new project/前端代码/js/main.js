...
function initBreathingExercise() {
    ...
    function stopBreathing() {
        ...
        // 保存练习记录 - 确保至少记录1秒的练习时长
        const totalDuration = Math.max(1, cycleCount * (inhaleTime + exhaleTime + 2));
        savePracticeRecord('正念呼吸', totalDuration);
    }
}
...
function initMeditation() {
    ...
    audio.addEventListener('error', (e) => {
        ...
        console.error('音频源:', audio.src);
        showNotification('音频文件加载失败，请检查文件路径', 'error');
    });
    
    audio.addEventListener('loadstart', () => {
        console.log('开始加载音频:', audio.src);
    });
    
    // 设置音频时长显示（不依赖音频元数据）
    function setAudioDuration() {
        const currentMeditation = getCurrentMeditationType();
        let targetDuration = 300; // 默认5分钟
        
        if (currentMeditation && meditationData[currentMeditation]) {
            targetDuration = meditationData[currentMeditation].duration;
            console.log('使用预定义时长:', targetDuration);
        }
        
        totalTimeEl.textContent = formatTime(targetDuration);
    }
    
    audio.addEventListener('canplay', () => {
        console.log('音频可以播放:', audio.src);
    ...
    // 获取当前冥想类型
    function getCurrentMeditationType() {
        const modalTitle = document.querySelector('.meditation-modal .modal-title');
        return modalTitle ? modalTitle.textContent.trim() : null;
    }
    ...
    function updateProgress() {
        ...
        const progress = (audio.currentTime / targetDuration) * 100;
    ...
    function togglePlay() {
        ...
        if (audio.paused) {
            ...
            console.log('使用预定义时长进行播放:', targetDuration);
            ...
            audio.play().then(() => {
                ...
                showNotification('音频播放失败: ' + error.message, 'error');
            });
        } else {
            audio.pause();
            isPlaying = false;
            ...
            if (data && audioSrc) {
                ...
                const loadTimeout = setTimeout(() => {
                    if (audio.readyState < 2) { // HAVE_CURRENT_DATA
                        ...
                        showNotification('音频文件加载超时，请检查网络连接', 'error');
                    }
                }, 5000);
...
function updateEmotionChart() {
    ...
    if (window.emotionChart) {
        try {
            ...
            } else if (typeof window.emotionChart.clear === 'function') {
                window.emotionChart.clear();
            }
        } catch (error) {
...
// 保存练习记录
// ========================================
async function savePracticeRecord(type, duration) {
    console.log('🎯 开始保存练习记录:', { type, duration });
    ...
    try {
        ...
        if (!token) {
            console.warn('⚠️ 未找到登录token，跳过服务器保存');
            return;
        }
        
        const response = await fetch('http://localhost:8081/api/practices', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                type: 'breathing',  // 正念呼吸练习
                name: type,         // 练习名称（如：正念呼吸）
                duration: Math.round(duration), // 练习时长（秒）
                description: `完成${type}练习，持续${Math.round(duration)}秒`,
                tags: '正念,呼吸,放松'
            })
        });
        ...
        if (response.ok) {
            ...
            console.log('✅ 练习记录已保存到服务器:', result);
            
            // 更新个人中心显示
            updatePersonalCenterStats();
        } else {
            console.error('❌ 服务器保存失败:', response.status, response.statusText);
        }
    } catch (error) {
...
// 保存测评记录
// ========================================
async function saveAssessmentRecord(name, score, level, analysis, factorScores = null) {
    console.log('🎯 开始保存测评记录:', { name, score, level });
    
    try {
        // 确定测评类型
        let type = 'apeskPstr';
        if (name.includes('SAS')) type = 'sas';
        else if (name.includes('SDS')) type = 'sds';
        else if (name.includes('BAI')) type = 'bai';
        else if (name.includes('PSQI')) type = 'psqi';
        else if (name.includes('DASS-21')) type = 'dass21';
        else if (name.includes('SCL-90')) type = 'scl90';
        ...
        const response = await fetch('http://localhost:8081/api/assessments', {
            ...
            body: JSON.stringify({
                ...
                level: level,
                analysis: analysis,
                factorScores: factorScores ? JSON.stringify(factorScores) : null,
        ...
        if (response.ok) {
            const result = await response.json();
            console.log('✅ 测评记录已保存到服务器:', result);
        } else {
            console.error('❌ 服务器保存失败:', response.status, response.statusText);
        }
        
        // 无论服务器保存是否成功，都更新个人中心显示（因为本地存储已成功）
        updatePersonalCenterStats();
        
        // 触发全局事件，通知其他页面数据已更新
        window.dispatchEvent(new CustomEvent('assessmentCompleted', {
            detail: { name, score, level, type }
        }));
    } catch (error) {
        console.error('❌ 保存测评记录时出错:', error);
    }
}
...
// 更新个人中心统计数据
// ========================================
async function updatePersonalCenterStats() {
    try {
        console.log('🔄 开始更新个人中心统计数据...');
        
        // 检查是否在个人中心页面
        if (window.location.pathname.includes('profile.html')) {
            // 如果在个人中心页面，调用页面内的函数
            if (typeof loadUserProfile === 'function') {
                console.log('📊 调用个人中心数据更新...');
                await loadUserProfile();
                console.log('✅ 个人中心数据已更新');
            } else {
                console.warn('⚠️ loadUserProfile 函数不存在');
            }
        } else {
            // 如果不在个人中心页面，只更新本地存储的数据
            console.log('📊 更新本地存储数据...');
            const assessments = JSON.parse(localStorage.getItem('assessmentHistory')) || [];
            console.log(`📊 当前测评记录数量: ${assessments.length}`);
        }
...
function initApeskPstrAssessment() {
    ...
    function showResult() {
        ...
        // 保存测评记录
        saveAssessmentRecord('APESK-PSTR 心理压力量表', totalScore, level, analysis);
    }
...
function initSasAssessment() {
    ...
    function showResult() {
        ...
        // 保存测评记录
        saveAssessmentRecord('SAS 焦虑自评量表', standardScore, level, analysis);
    }
...
function initSdsAssessment() {
    ...
    function showResult() {
        ...
        // 保存测评记录
        saveAssessmentRecord('SDS 抑郁自评量表', standardScore, level, analysis);
    }
...
function initBaiAssessment() {
    ...
    function showResult() {
        ...
        // 保存测评记录
        saveAssessmentRecord('BAI 贝克焦虑测试', totalScore, level, analysis);
    }
...
function initPsqiAssessment() {
    ...
    function showResult() {
        ...
        // 保存测评记录
        saveAssessmentRecord('PSQI 匹兹堡睡眠质量指数', totalScore, level, analysis);
    }
...
function initDass21Assessment() {
    ...
    function showResult() {
        ...
        saveAssessmentRecord('DASS-21 抑郁焦虑压力量表', Math.max(depression, anxiety, stress), '多维度评估', analysis, factorScores);
    }
...
function initScl90Assessment() {
    ...
    function showResult() {
        ...
        // 保存测评记录
        saveAssessmentRecord('SCL-90 心理健康自评量表', totalScore, level, analysis, factorScores);
    }
